<x-admin-template>
    <div class="flex mx-3 justify-center mt-6 sm:px-6 lg:px-8">
        <div>
            ...
            <div>
                <form class="mt-8 space-y-6" action="{{ route('line-free.update', $lineFree) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="md:grid md:grid-cols-2 gap-6 mt-4">
                        <div class="mt-5 md:mt-2">
                            <label for="">Label<span class="text-red-500">*</span></label>
                            <input id="label" name="label" type="text" value="{{ $lineFree->label }}"
                        ...
                        <div class="mt-5 md:mt-2">
                            ...
                            <select name="type" id="type"
                                ...
                                <option selected value="{{ $lineFree->type }}">{{ $lineFree->type }}</option>
                        ...
                        <div class="mt-5 md:mt-2">
                            ...
                            <input type="number" id="purchase_quantity" name="purchase_quantity"
                                value="{{ $lineFree->purchase_quantity }}"
                                class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm">
                        </div>

                        <div class="mt-5 md:mt-2">
                            <label for="">Free Quantity <span class="text-red-500">*</span></label>
                            <input type="number" id="free_quantity" name="free_quantity"
                                value="{{ $lineFree->free_quantity }}"
                                class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm">
                        </div>

                        <div class="mt-5 md:mt-2">
                            <label for="">Lower Limit <span class="text-red-500">*</span></label>
                            <input type="number" id="lower_limit" name="lower_limit"
                                value="{{ $lineFree->lower_limit }}"
                                class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm">
                        </div>

                        <div class="mt-5 md:mt-2">
                            <label for="">Uper Limit <span class="text-red-500">*</span></label>
                            <input type="number" id="uper_limit" name="uper_limit" value="{{ $lineFree->uper_limit }}"
                                class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm">
                        </div>
                    </div>
